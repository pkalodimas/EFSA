...
/**
 * Import the term applicabilities contained in the term sheet into the
 * database. This class can manage also new terms which could be possibly
 * created in the {@link TermSheetImporter#importSheet()}. In fact, here we
 * manage also the possibility that a parent of a new term can be also a new
 * term itself.
 * 
 * @author avonva
 * @author shahaal
...
public class ParentImporter extends SheetImporter<Applicability> {
	...
	public ParentImporter(CatalogueRelationDAO<Applicability, Term, Hierarchy> dao, Catalogue catalogue)
			throws SQLException {
		this.dao = dao;
	 ...
	 * parent importer class. In particular, if a term with code or parent code
	 * containing {link CodeGenerator#TEMP_TERM_CODE} is encountered, its real code
	 * is taken from the {@code newCodes} hashmap (which was created before in the
	 * {@link TermSheetImporter#importSheet()} if new terms were encountered!)
	 * 
	 * @param newCodes
	 */
	...
	public Collection<Applicability> getAllByResultSet(ResultDataSet rs) throws ImportException {
		...
		for (Hierarchy hierarchy : hierarchies) {
			...
			String parentCode = rs.getString(getHierarchyFieldName(hierarchy, Headers.SUFFIX_PARENT_CODE));

			// if temp code we need to get the real code
			// of the term
			if (newCodes != null && generator.isTempCode(parentCode)) {
			...
			if (parentCode.equals(termCode)) {
				ImportException e = new ImportException(
						"ERROR: A TERM CANNOT BE PARENT OF ITSELF: term code " + termCode, "X101");
				e.setData(termCode);
				throw e;
			}
			...
			// get the term order
			int order = rs.getInt(getHierarchyFieldName(hierarchy, Headers.SUFFIX_ORDER), -1);

			// get the term reportability
			boolean reportable = rs.getBoolean(getHierarchyFieldName(hierarchy, Headers.SUFFIX_REPORT), true);

			Applicability appl = createApplicability(isRoot, termId, parentId, hierarchy, order, reportable);
	 ...
	 * @return
	 */
	private Applicability createApplicability(boolean isRoot, int childId, Integer parentId, Hierarchy hierarchy,
			int order, boolean reportable) {
	...
	/**
	 * Get the name of the column which contains the data related to the field
	 * "field". Example, we want the flag column related to the Reporting hierarchy
	 * => it returns reportingFlag if the hierarchy is the master, the convention
	 * says that we should search with the master keyword! In the previous example
	 * we would search masterFlag. The master code is variable, so we need this
	 * check.
	 * 
	 * @param hierarchy
	 * @param field
	...
	public void end() {
		...
		for (Applicability appl : tempAppl) {
			...
			if (order == -1) {
				...
				int newOrder = parentDao.getNextAvailableOrder(parent, hierarchy);
