...
/**
 * Base class for a term which was already described before (code is a full code
 * with zero or more facets)
 * 
 * @author avonva
 * @author shahaal
 *
...
public class DescribedTerm {
	...
	public Term getTerm() {
		...
		// create the base term copy (to avoid overriding things! If we act directly to
		// the base term
		...
		baseTermCopy.setDisplayAs(baseTerm.getShortName(false));
		...
		for (String facetFullCode : getFullFacetCodes()) {
			...
			TermAttribute ta = new TermAttribute(baseTermCopy, attrDao.getByCode(facetHeader), facetFullCode);
	...
	/**
	 * Check if the term "term" is contained in the implicit or explicit facets of
	 * the described term
	 * 
	 * @param term
	 * @return
	...
	/**
	 * Check if all the terms referred by this described term are present into the
	 * database or not.
	 * 
	 * @return
	 */
