...
public abstract class FormObjectsList<T> {
	 ...
	 */
	public FormObjectsList(Shell shell, String windowCode, String title, Collection<T> objs, boolean multiSel) {
	...
	/**
	 * Create the table viewer which contains the catalogues information The table
	 * contains also several columns to display different fields (customizable
	...
	private TableViewer createTable(Composite parent, String[] columns) {
		...
		// SWT.FULL_SELECTION is set to select the entire row of the table,
		// independently of the selected column
		// This was done since we have to select a single catalogue, not a catalogue
		// column

		// we create a check box table viewer for multiple selection, otherwise we
		// create a standard
		// single selection table
		final TableViewer table;
		if (multiSel)
			table = CheckboxTableViewer.newCheckList(dialog,
					SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.FULL_SELECTION);
		else
			table = new TableViewer(dialog, SWT.BORDER | SWT.V_SCROLL | SWT.H_SCROLL | SWT.FULL_SELECTION | SWT.SINGLE);

		// set the content provider of the table
		table.setContentProvider(new ObjectContentProvider<T>());

		// set the layout data for the table (note: these will be used also for the
		// dialog)
	...
	/**
	 * Call the Class listener if it was set (i.e. call the general listener of the
	 * FormCataloguesList class) The listener event will contain the selected
	 * catalogues as array list if multiSel = true. Otherwise, it will contain the
	 * only selected catalogue as Catalogue if multisel=false
	 * 
	...
	/**
	 * Get the selected catalogues, works only with tableviewer with single or multi
	 * selection
	 * 
	 * @param viewer
	 * @return
	...
	/**
	 * Perform the actions needed to select a catalogue (download), that is, get the
	 * selected catalogue and call the Class listener to inform outside the class
	 * that a catalogue has been selected.
	 * 
	 * @param dialog
	 * @param viewer
	 */
	...
	/**
	 * Create the listener for a ok action (i.e. a catalogue is selected) NOTE: We
	 * cannot pass as parameter only the ISelection instead of the viewer! In fact,
	 * when if the selection changes the ISelection object would not be updated!
	 * Only the viewer is updated (and its selection)
	 * 
		...
		IDoubleClickListener listener = new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent arg0) {
				...
				if (multiSel) {
					...
					/*
					 * 
					 * // get the selected elements from the table Object element = (
					 * (IStructuredSelection) viewer.getSelection() ) .getFirstElement();
					 * 
					 * 
					 * // get if it is checked or not boolean isChecked = ( (CheckboxTableViewer)
					 * viewer ).getChecked( element );
					 * 
					 * // invert the check ( (CheckboxTableViewer) viewer ).setChecked( element,
					 * !isChecked );
					 */
