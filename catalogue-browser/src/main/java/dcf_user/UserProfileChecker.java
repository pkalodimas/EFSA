...
/**
 * Class to ask to the DCF the users access level. In particular: if the user
 * username appears in the catusers catalogue then the user can edit the related
 * catalogues which are listed as attribute. Otherwise, the user can only read
 * the catalogue
 * 
 * @author avonva
 */
...
public class UserProfileChecker extends Thread {
	...
	public void setAccessLevel() {
		...
		downloader.setDoneListener(new ThreadFinishedListener() {
			...
			public void finished(Thread thread, int code, Exception e) {
				...
				case OK:
					if(user.isLoggedInOpenAPI()) {
						// set as catalogue viewer
						LOGGER.info("User access level: catalogue viewer");
						user.setUserLevel(UserAccessLevel.DATA_PROVIDER);
					} else {
						// set as catalogue manager
						LOGGER.info("User access level: catalogue manager");
						// update the editable catalogues based on username
	...
	/**
	 * Get all the editable catalogues codes for the current catalogue manager user
	 * 
	 * @return
	 */
	private ArrayList<String> getEditableCataloguesCodes() {
		...
		Catalogue users = catDao.getLastVersionByCode(Catalogue.getCatUsersCatalogue().getCode(), Dcf.dcfType);
	...
	/**
	 * Get the catalogue attribute related to the user access levels
	 * 
	 * @param catalogue the catalogue containing the user access level attribute
	 * @return
	 */
	private TermAttribute getEditCatAttribute(Catalogue catalogue) {
		...
		if (userTerm == null) {
			LOGGER.error(
					"USER " + user.getUsername() + ": Found catalogue manager account but the " + catalogue.getCode()
							+ " permissions catalogue does not contain it. Please add this account to the "
